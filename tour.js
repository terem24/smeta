...
// работы → деньги → сохранение → ссылка клиенту → документы и счёт. Каждый шаг
// подсвечивает свой элемент, объясняет словами, зачем он нужен, и показывает
// пальцем, что сделать.
//
// Шаги, которые видит не каждый (умное заполнение, распознавание, «Деньги», панель
// разделов, документы), пропускаются сами по скрытому элементу — отдельных проверок
// тарифа и доступа здесь нет намеренно, иначе правило пришлось бы держать в двух
// местах и оно бы разъехалось с интерфейсом.
//
...
const Tour = {
    ...
    STEPS: [
        ...
        },
        {
            ...
            // Кубок из перечня убран: пилот рейтинга свёрнут общим выключателем
            // (RATING_ENABLED в gamification.js), кнопки в шапке нет ни у кого.
            // Вернут рейтинг — вернуть и упоминание, здесь и в варианте продавца.
            text: 'Слева направо: конверт — сообщения и уведомления; дискета — сохранить смету в облако; стрелки «‹ ›» — открыть смету по коду, которым с вами поделились; круговая стрелка — сбросить расчёт и начать с чистого листа; луна — светлая или тёмная тема; шапочка выпускника — эти самые подсказки. Наведите на любую — подскажет, что делает. Рядом идут суммы оборудования и монтажа: они пересчитываются на каждое изменение. На телефоне весь ряд спрятан в меню ☰.',
        ...
        {
            // Плавающая кнопка со звёздочкой. Гостю её не показывают, и на вкладках
            // кроме «Оборудования» тоже (см. app.syncAiFabVisibility) — тогда шаг
            // пропустится сам. Про то, как оно устроено внутри (разбор фразы по
            // правилам, не языковая модель), человеку знать незачем: ему важно, что
            // писать можно как говорится.
            key: 'aifill', mob: 'inputs',
            sel: '#ai_parse_fab_btn',
            title: 'Заполнить одной фразой',
            text: 'Кнопка «ИИ-заполнение» у края экрана: продиктуйте или впишите объект как рассказали бы на словах — «двести квадратов под Новосибирском, газ, тёплый пол на первом этаже, бойлер на четверых», — и параметры расставятся сами. Останется проверить и поправить. Кнопку можно перетащить в удобное место долгим нажатием.',
            anim: { type: 'press', label: '✨ ИИ-заполнение' }
        },
        {
        ...
        {
            // Вкладка открыта не всем: доступ раздаётся лично, по дистрибьютору или
            // по региону (app.syncUI зовёт RecognizeUI.syncButton). Кому не открыта —
            // у того шаг пропустится по скрытому элементу.
            key: 'recognize', mob: 'output',
            sel: '#tab_recognize',
            title: 'Чужая смета в свой расчёт',
            text: 'Вкладка «Распознавание» принимает готовый документ: смету конкурента, накладную поставщика или план этажа. Позиции подбираются по каталогу и прайсу, спорные строки вы проверяете на отдельном экране и уже оттуда переносите в свой расчёт. С плана этажа снимаются комнаты с площадями — они лягут в подробный расчёт.',
            anim: { type: 'press', label: '3. Распознавание' }
        },
        {
            // «Деньги» — только Профи и только монтажнику (см. app.syncMoneyTab).
            // Базовому вкладки нет вовсе, поэтому шаг у него не покажется.
            key: 'money', mob: 'output',
            sel: '#tab_money',
            title: 'Что останется вам',
            text: 'Вкладка «Деньги» считает вашу прибыль по объекту. Один раз задайте закупочные скидки по маркам, долю бригады в расценке и накладные — доставку, выезды, налог с оборота. Дальше видно маржу целиком, по разделам сметы и по каждой позиции, а карточка «Уступить» показывает, во что обойдётся скидка клиенту. Вкладка работает на тарифе Профи.',
            anim: { type: 'press', label: '4. Деньги' },
            done: () => app.state.viewMode === 'money',
            seller: { skip: true }
        },
        {
        ...
        {
            // Документы живут не под сметой, а внутри карточки заказа, и найти их
            // сами не догадывались. Подсвечиваем вход — пункт «Заказы» в панели
            // разделов; у гостя и на телефоне панели нет, там шаг пропустится, как и
            // соседний шаг про саму панель.
            key: 'docs',
            sel: '.lk-rail-item[data-rail="orders"]',
            title: 'Договор и акты',
            text: 'Отправленная клиенту смета становится заказом в разделе «Заказы», и у каждого заказа есть кнопка «Документы». Оттуда выходит комплект: договор бытового подряда, спецификация оборудования, смета работ и акт сдачи-приёмки. Стороны, объект, состав и цены подставляются из расчёта, руками остаётся дописать сроки и порядок оплаты. Цена по умолчанию твёрдая — так же считает и закон, если в договоре не сказано иначе, и от подорожания каталога она уже не поедет.',
            anim: { type: 'press', label: '📄 Документы' },
            // Договор подряда — про монтаж, продавец его не заключает.
            seller: { skip: true }
        },
        {
            key: 'send', mob: 'output',
        ...
        {
            // Третья кнопка в ряду под сметой. О ней обучение не говорило вовсе, а
            // это единственный путь от расчёта к оплате.
            key: 'invoice', mob: 'output',
            sel: '#btn_invoice_trigger',
            title: 'Запросить счёт',
            text: 'Кнопка отправляет смету вашему менеджеру, чтобы он выставил счёт на оплату. Заказ после этого виден в разделе «Заказы» вместе со статусом: отправлено, согласовано, оплачено. Для отправки в профиле должны стоять имя, телефон, город и почта — без них менеджеру некуда ответить.',
            anim: { type: 'press', label: 'Запросить счёт' },
            last: true
        }
    ],
    ...
    // Шага у этого человека не будет вовсе: либо он убран по сфере деятельности,
    // либо его элемент спрятан насовсем — нет тарифа («Деньги»), нет доступа
    // (распознавание), не вошёл (умное заполнение). Смотрим только на инлайновый
    // display: его ставит app.js, когда прячет вкладку или кнопку по правам.
    //
    // Скрытое вёрсткой — другая вкладка телефона, узкий экран — сюда намеренно не
    // попадает: такие шаги показываются, стоит переключить вкладку, и вычитать их
    // из счётчика значило бы менять «из скольки» на каждом движении пальцем.
    unavailable: function (s) {
        if (this.skipped(s)) return true;
        let el = null;
        try { el = typeof s.sel === 'function' ? s.sel() : document.querySelector(s.sel); } catch (e) { return false; }
        if (!el) return true;
