...
const RecognizeMatch = (function () {
   ...
   * Дымоход ли это.
   *
   * Модель зовёт «Отвод 90° утепленный традиц. D80(п-м)» канализационным
   * отводом: других отводов в её словаре типов не было, а восьмидесятый
   * диаметр у канализации действительно бывает. Признаки самого дымохода
   * при этом однозначны — слово «дымоход», сокращение «традиц.» из прайса,
   * «(п-м)» (папа-мама) и коаксиальные пары размеров 60/100, 80/125, 80/80.
   *
   * Смотрим и на подобранную позицию: у неё есть имя листа прайса («STOUT
   * Дымоходы») и артикул серии — SCR- у STOUT, RCA- у ROMMER.
   */
  const CHIMNEY_WORDS = /дымоход|коаксиальн|газоход|воздуховод|дымов[а-яё]*\s+(труб|канал)/;
  function isChimney(item) {
    if (!item) return false;
    const m = item._m && item._m.item;
    const s = [item.raw, m && m.name, m && m.s, m && (m.id || m.a)]
      .filter(Boolean).join(' ').toLowerCase();
    if (!s) return false;
    if (CHIMNEY_WORDS.test(s)) return true;
    // «традиц.» и «(п-м)» встречаются только в дымоходной номенклатуре.
    // «конденсац.» сюда не берём: так же сокращают и котёл.
    if (/традиц|\(п-м\)|\(м-п\)/.test(s)) return true;
    // Коаксиальная пара размеров — у канализации таких не бывает.
    if (/(^|[^0-9])(60 ?\/ ?100|80 ?\/ ?125|80 ?\/ ?80)([^0-9]|$)/.test(s)) return true;
    // Артикулы дымоходов обоих брендов.
    if (/(^|[^a-z])(scr|rca)-\d/.test(s)) return true;
    return false;
  }
  ...
  function systemOf(item, hint) {
    ...
    //
    // Дымоход из этой ветки выведен: строку «Отвод 90° утепленный традиц.
    // D80(п-м)» модель приносит с типом «отвод_канализация», и подбор уходил
    // искать раструбный отвод вместо дымоходного колена.
    if (/канализац|ревизия/.test(t) && !isChimney(item)) return 'sewer';
  ...
  function matchPrice(rec) {
    ...
    const sewerType = /канализ|ревизия/.test(type) && !isChimney(rec);
  ...
  const CATALOG_SECTIONS = {
    ...
    // Дымоход поштучно: отводы, удлинители, адаптеры, оголовки. В смете они
    // стоят там же, где комплект целиком, — в котельной.
    chimney_trad_60100: '2. Обвязка котельной',
    chimney_cond_60100: '2. Обвязка котельной',
    chimney_cond_80125: '2. Обвязка котельной',
    chimney_split_d80: '2. Обвязка котельной',
  ...
  function guessSectionRule(item, profile) {
    ...
    // Дымоход поштучно: в каталоге калькулятора его нет (там только готовые
    // комплекты), в прайсе он лежит на листе «STOUT Дымоходы». Стоит такая
    // деталь всегда в котельной. Проверка идёт до канализационной: модель
    // приносит «Отвод 90° утепленный традиц. D80(п-м)» с типом
    // «отвод_канализация», и колено дымохода за 2 900 ₽ оседало в разделе
    // канализации, а удлинители — в «Дополнительных материалах».
    if (isChimney(item)) return { section: '2. Обвязка котельной', sure: true };
