 ...
 * пробыл.
 *
 * ЧТО СЧИТАЕТСЯ ВРЕМЕНЕМ. Только активное: вкладка на переднем плане И за последние две
 * минуты была мышь, клавиатура, прокрутка или касание. Открытая с утра и забытая вкладка
 * даёт ноль. Иначе у продавца, который не закрывает калькулятор весь день, набегало бы
 * восемь часов, и средняя цифра по базе не значила бы ничего.
 *
 * Почему две минуты, а не одна. С минутой терялось время того, кто внимательно читает
 * смету или сверяет позиции, не трогая мышь: первый живой визит владельца дал 85 секунд
 * при трёх-четырёх минутах работы. Две минуты такое чтение покрывают, а забытую
 * вкладку по-прежнему отсекают — она накрутит не больше двух минут за раз.
 *
...
window.SessionTrack = {
    ...
    IDLE_MS: 2 * 60 * 1000,          // столько без действий — секунды не капают
    ...
    init: function () {
        ...
        // первые минуты после возвращения таймер бы стоял.
