...
(function () {
    ...
    function show() {
        if (shown) return;
        // Баннер про cookie стоит ровно там же, внизу экрана. Две карточки стопкой
        // выглядят как навязчивая реклама, да и согласие важнее: ждём, пока на него
        // ответят, и пробуем снова. Обычно вопрос закрыт ещё в первый заход, а плашка
        // раньше второго не показывается — так что это случай редкий, но неприятный.
        if (document.querySelector('.hc-cookie-banner')) {
