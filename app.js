    ...
    init: function () {
        try {
            ...
            if (localStorage.getItem('stout_save')) {
                try { 
                    let saved = JSON.parse(localStorage.getItem('stout_save'));
            ...
            // Санация состояния (State Sanitization)
            if (!Array.isArray(this.state.fuels)) this.state.fuels = ['el'];
            if (this.state.fuels.length === 0) this.state.fuels = ['el'];
            if (!Array.isArray(this.state.systems)) this.state.systems = [];
            this.state.area = parseFloat(this.state.area) || 150;
            this.state.region = parseInt(this.state.region) || 100;
            this.state.mat = parseFloat(this.state.mat) || 1.0;
            this.state.res = parseInt(this.state.res) || 3;
            if (!this.state.viewMode) this.state.viewMode = 'equipment';
