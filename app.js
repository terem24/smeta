...
const app = {
    ...
    addCustomEqPrompt: function() {
        ...
        this.state.userAddedEq.push({
            ...
            price: price,
            q: qty,
            brand: "" // Бренд пустой для скрытия
        });
        this.saveState();
        this.render();
    },
    ...
    render: function () {
        ...
        if (this.state.userAddedEq && this.state.userAddedEq.length > 0) {
            this.state.userAddedEq.forEach(eq => {
                // Добавляем красный крестик для удаления
                let customName = `<span class="work-del-btn" style="color:#EF4444; font-weight:bold; cursor:pointer; margin-right:8px;" onclick="event.stopPropagation(); app.deleteEq('${eq.id}')">✖</span>${eq.name}`;
                
                // Передаем пустой бренд для прозрачной иконки
        ...
        // Кнопка добавления (в стиле монтажных работ)
        h += `<tr class="hide-custom-eq-btn"><td colspan="7" style="padding:15px; text-align:center;">
