...
const app = {
    ...
    render: function () {
        ...
        // === ОБНОВЛЕНИЕ СУММ В ЛИПКОЙ ШАПКЕ ===
        let headerTotals = document.getElementById('header_totals');
        if (headerTotals) {
            let eqHtml = `<span style="color:var(--text-sec); font-size:11px; margin-right:4px;">Оборудование:</span> <b style="color:var(--primary); font-size:14px;">${app.lastEqSum.toLocaleString()} ₽</b>`;
            let wksHtml = "";
            // Добавляем монтаж только если у пользователя PRO-доступ
            if (this.state.accountType === 'pro') {
                wksHtml = `<span style="margin:0 10px; color:var(--border);">|</span> <span style="color:var(--text-sec); font-size:11px; margin-right:4px;">Монтаж:</span> <b style="color:#F97316; font-size:14px;">${app.lastWorksSum.toLocaleString()} ₽</b>`;
            }
            headerTotals.innerHTML = eqHtml + wksHtml;
