    ...
    filterAdminData: function (query) {
        ...
        rows.forEach(row => {
            ...
            renderScheme: function() {
                ...
                // 1. Базовый слой (добавляется всегда первым)
                layers.push('bg_frame.png');

                // 2. Газовый котел
                if (s.fuels.includes('gas')) {
                    layers.push('boiler_gas.png');
                    layers.push('boiler_gas_piping.png');
                }

                // 3. Электрокотел
                if (s.fuels.includes('el')) {
                ...
                // 5. Рециркуляция (только если есть ГВС)
                if (s.hotWater === true && s.recirc === true) {
                ...
                // Расчет количества насосных групп/контуров
                const circuitsCount = s.systems.length + (s.hotWater ? 1 : 0);

                // 8. Дополнительная группа РО (3 или более групп)
                if (circuitsCount >= 3) {
                    layers.push('system_rad_2.png');
                }

                // 9. Гидрострелка (2 или более контура)
                if (circuitsCount >= 2) {
                    layers.push('hydro_manifold.png');
                }

                // Генерация HTML
                let html = `<div id="dynamic_scheme" style="position: relative; width: 100%; padding-bottom: 75%; overflow: hidden;">`;
                
                layers.forEach(layer => {
                    html += `<img src="${basePath}${layer}" alt="${layer}" style="position: absolute; top: 0; left: 0; width: 100%; height: auto;">`;
                });
