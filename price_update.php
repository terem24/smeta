<?php
/**
 * Пересборка индекса прайс-листа ТЕРЕМ (проверка раз в неделю).
 *
 * Запускается планировщиком Beget раз в неделю, по понедельникам в 4:00
 * (cron «0 4 * * 1»): прайс ТЕРЕМ бывает перевыложен посреди месяца под тем же
 * именем. Лишний запуск стоит один короткий запрос — если файл на сервере ТЕРЕМ
 * не менялся, сборщик сразу выходит. Что делает:
 ...
 * Версия сборщика — $BUILDER_VERSION ниже, она же в первой строке журнала.
 */
 ...
 */
// Версию менять при КАЖДОЙ правке разбора: по ней в журнале видно, какой файл
// реально работает на Beget. 22.09.2026 пересборка шла старым сборщиком, а по
// журналу это было не отличить — строка у обеих версий была одинаковой.
$BUILDER_VERSION = '22.09.2026 №2 (дорожки: таблицы рядом в строке)';
logmsg('--- запуск обновления прайса (сборщик ' . $BUILDER_VERSION . ') ---');
...
foreach ($sheets as $s) {
    ...
    $mapCols = static function ($rows, $hi) use ($reArt, $reName, $rePrice, $reCanon, $reCanonSp, $isDiscount) {
        ...
        /**
         * Вторая колонка артикулов — запасная, для строк с пустой первой.
         *
         * В том же листе нержавейки ROMMER «Артикул 1» (RSS-0001-…) заполнен
         * только у ходовых позиций, а у штанг 2 м, российских штанг 4 м и
         * больших диаметров стоит один «Артикул 2» (RSS-1001-200022). Такие
         * строки уходили в индекс вовсе без кода, и найти их по артикулу было
         * нельзя (22.09.2026). Первую колонку не трогаем — берём вторую только
         * там, где в первой пусто.
         */
        $cArt2 = null;
        if ($seenRows > 0 && $canon) {
            foreach ($canon as $col => $n) {
                if ($col !== $cArt && $n * 2 >= $seenRows) { $cArt2 = $col; break; }
            }
        }
